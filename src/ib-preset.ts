export const IB_Preset = {
  ...
  "DelCheck": {
    ...
    OnChange: function (evtParam: any) {
      ...
      if (evtParam.row.Added) {
        ...
        var render = rows.length > 1 && evtParam.sheet.MainCol ? true : false;
        //모두 체크하고 편집 불가로 변경
        for (var i = 0; i < rows.length; i++) {
          var row = rows[i];
          evtParam.sheet.setValue(row, evtParam.col, chked, render, true);
  ...
  "TreeSumFormula": function(fr: any) {
    var sum = 0;
  ...
  "TreeAvgFormula": function(fr: any) {
    var avg = 0,
      sum = 0,
      count = 0;
  ...
  "TreeCountFormula": function(fr: any) {
    var count = 0;
  ...
  "TreeMaxFormula": function(fr: any) {
  ...
  "TreeMinFormula": function(fr: any) {
